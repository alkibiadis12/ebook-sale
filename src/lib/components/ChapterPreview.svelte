...
<section
	class="container mx-auto justify-items-center overflow-x-hidden pb-10 pt-10 lg:grid lg:grid-cols-2 lg:pb-36 lg:pt-36 2xl:pb-36"
>
	<div class="font-anton">
		<h2 class="text-center text-4xl md:text-5xl lg:text-start">What you' re getting</h2>
		<ul
			class=" mt-10 flex flex-col items-center shadow-md md:mx-auto md:w-fit
		 lg:w-auto"
		>
			{#each chapters as chapter (chapter)}
				<li
					class=" border-b-2 border-gray-200 {expanded === chapter.id
						? 'bg-black text-white'
						: 'text-black hover:bg-gray-200 '}"
				>
					<button
						class="text-md w-96 py-3 lg:text-lg"
						aria-controls={`chapter-info-${chapter.id}`}
						aria-expanded={expanded === chapter.id}
						onclick={() => handleClick(chapter)}
					>
						<h3>Chapter {chapter.id}:{chapter.buttonText}</h3>
					</button>
				</li>
			{/each}
		</ul>
